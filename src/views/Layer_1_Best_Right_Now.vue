  ...
  <div class="dashboard-container">
    <div class="glass-header">
      <div class="header-main-content">
        <h1 class="main-glow-title">Poppy's Right Now</h1>
        <div class="layer-badge">LAYER 1 — PERSONALIZED OBJECTS</div>
      </div>

      <div class="intro-box">
        <p class="intro-main">
          Your personalized cosmic recommendations, tailored to your context and preferences.
        </p>
        <p class="intro-details">
          This layer evaluates moons, stars, and planets using your <strong>location</strong>, 
          the <strong>current date and time</strong>, and your <strong>liked objects</strong>. 
          Each object receives a <strong>matching score</strong> through a weighted, rule-based system 
          that considers factors such as brightness and prior interactions.
          <br><br>
          Recommendations also include a <strong>visibility percentage</strong>, indicating how likely 
          the object is to be observable from your sky. This information is <em>informational only</em> 
          and does not influence the matching score.
        </p>
      </div>
    ...
    <div v-else class="results-area">
        ...
        >
          ...
          <div class="card-content">
            ...
            <div class="info">
              ...
              <button class="details-link" @click="goTo(item)">
                Details ✨
              </button>
            </div>
          </div>
      ...
      <div v-if="filteredRecommendations.length === 0" class="centered-state">
        <div class="status-content">
          <p class="empty-msg">No celestial objects found for this time or place. 🔭</p>
...
export default {
  ...
  setup() {
    ...
    const fetchRecommendations = async () => {
      try {
        ...
        if (res.data.success && res.data.results) {
          ...
          rawData.value = {
            stars: (data.Stars || []).map(item => ({ ...item, type: 'stars' })),
            planets: (data.Planets || []).map(item => ({ ...item, type: 'planets' })),
            moons: (data.Moons || []).map(item => ({ ...item, type: 'moons' }))
          };
    ...
    /**
     * PLANET REALISM UPDATE
     * Maps planet names to real astronomical colors
     */
    const getPlanetStyle = (item) => {
      ...
      const planetColors = {
        ...
        'ceres':   { bg: '#a3a3a3', glow: '#5c5c5c' },
        'eris':    { bg: '#f1f1f1', glow: '#a1a1a1' },
        'haumea':  { bg: '#e5e5e5', glow: '#999' },
        'makemake':{ bg: '#b65d3a', glow: '#7d381c' }
      };
    ...
    const getPlanetTexture = (item) => {
      ...
      if (['mars', 'mercury', 'ceres', 'pluto'].includes(name)) return 'dusty';
