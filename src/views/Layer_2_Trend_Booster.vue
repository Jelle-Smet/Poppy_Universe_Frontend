  ...
  <div class="dashboard-container">
    <div class="glass-header">
      <div class="header-main-content">
        <h1 class="main-glow-title">Poppy's Trend Booster</h1>
        <div class="layer-badge secondary">LAYER 2 — TREND-BASED RECOMMENDATIONS</div>
      </div>

      <div class="intro-box trend-style">
        <p class="intro-main">
            Layer 2 is the <strong>trend-based recommendation layer</strong>. It highlights 
            celestial objects that are currently gaining attention across the platform.
        </p>

        <div class="intro-details">
            <p>
            This layer analyzes <strong>global user interaction patterns</strong> to detect 
            which objects are trending. Popularity is applied as a <strong>controlled ranking adjustment</strong> 
            on top of existing personalized results, ensuring trends enhance — but never override — 
  ...
  setup() {
    ...
    const fetchRecommendations = async () => {
      try {
        ...
        const res = await axiosWithAuth.post('/api/engine/run-l1-l2', payload); 
        ...
        if (res.data.success && res.data.results) {
          ...
          const processItems = (items, type) => (items || []).map(item => ({ 
            ...
            // UPDATED KEY: Boost_Amount_Pct
            Boost_Amount_Pct: item.Boost_Amount_Pct || 0
          }));
